...
Deno.serve(async (req) => {
  ...
  // 2. Query reminder callbacks: pending + scheduled_time between now and now+20min
  //    AND reminder_sent_at IS NULL (prevent duplicate reminders)
  const now = new Date()
  ...
  const { data: reminderCallbacks, error: reminderError } = await supabase
    ...
    .lte('scheduled_time', twentyMinLater.toISOString())
    .is('reminder_sent_at', null)

  if (reminderError) {
    console.error(
      'fire-callback-webhooks: reminder query error',
      reminderError,
    )
  }
  ...
  for (const row of (reminderCallbacks ?? []) as unknown as CallbackRow[]) {
    ...
    if (updateError) {
      console.error(
        'fire-callback-webhooks: failed to set reminder_sent_at',
        row.id,
        updateError,
      )
    }
